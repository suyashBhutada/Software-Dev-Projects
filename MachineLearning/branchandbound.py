...
def criterion_function(features):
    return pow(3, np.linalg.norm(features))
...
def display_tree(node, dot_object, parent_index):
    ...
    dot_object.node(str(node.index), "Selected features " + str(node.features) + "\nvalue = " + str(node.J) + "\nfeatures preserved = " + str(node.preserved_features))
...
def main(argv):
    ...
    dot.render("branch_tree", view=True)
